...
import { EllipsisIcon, ChevronDownIcon, PencilIcon, Trash2Icon, DownloadIcon, FileTextIcon, SheetIcon, ImageIcon, FilePlusIcon, CalendarIcon } from 'lucide-vue-next';
...
const toggleCheckbox = (checked: boolean, id: number) => {
    if (checked) {
        ...
        selectedTransactions.value = selectedTransactions.value.filter(userId => userId !== id);
    }
};
...
const deleteSelectedTransactions = () => {
    console.log("Delete transactions with ids: " + selectedTransactions.value.join(", "));
...
const exportTransactions = () => {
    const transactionsToExport = selectedTransactions.value.length > 0
        ? selectedTransactions.value
...
<template>
    ...
    <DashboardLayout>
        <template #header>
            <h1 class="text-xl font-semibold tracking-tight">Pendapatan Transferan</h1>
        </template>
        ...
        <section class="bg-white p-3 rounded-lg mb-3 dark:bg-neutral-800">
            <div :class="['flex justify-between items-center', { 'mb-3': showContentTwo }]">
                <div>
        ...
        <section class="bg-white p-3 rounded-lg dark:bg-neutral-800">
            <div class="mb-3 flex items-center justify-between">
                <p v-if="selectedCount > 0" class="font-semibold tracking-tight text-lg">({{ selectedCount }}) transaksi
                    dipilih
                </p>
                        ...
                        <TableRow v-for="(transaction, index) in transactions" :key="index">
                            <TableCell class="p-3">
                                <Checkbox :id="`transaction-${transaction.id}`"
                                    ...
                                    @update:checked="(checked) => toggleCheckbox(checked, transaction.id)" />
                            </TableCell>
                            <TableCell>{{ index + 1 }}</TableCell>
                            <TableCell>{{ transaction.duta }}</TableCell>
                            <TableCell>{{ transaction.donatur }}</TableCell>
                            <TableCell>{{ useDateFormat(transaction.transfer_date, 'HH:mm, DD MMM YYYY', {
                                locales:
                                    clientLocale
                            }) }}</TableCell>
