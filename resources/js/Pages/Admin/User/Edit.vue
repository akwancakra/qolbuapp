...
import { CornerUpLeftIcon, Loader2 } from 'lucide-vue-next';
...
function initializeForm(user: User) {
...
async function onSubmit(values: any) {
    ...
    try {
        Inertia.put(route('admin.users.update', props.user.id), values, {
            onFinish: () => {
...
// Inisialisasi data pengguna saat komponen di-mount
onMounted(() => {
...
<template>
    ...
    <DashboardLayout>
        <template #header>
            <h1 class="text-xl font-semibold tracking-tight">Edit User</h1>
        </template>
        ...
        <section class="bg-white p-3 rounded-lg mb-3 dark:bg-neutral-800">
            <div class="mb-3">
                ...
                <p class="text-sm text-neutral-500">Please fill in the details below to update the user.</p>
            </div>
                    ...
                    <Form id="userForm" :validation-schema="validationSchema" @submit="onSubmit"
                        ...
                        <div>
                            ...
                            <Field v-model="formValues.role" name="role" type="select">
                                <template v-slot="{ field }">
                                    <Select v-model="selectedRole" @update:model-value="formValues.role = $event">
                                        <SelectTrigger>
                                            <SelectValue :placeholder="selectedRole || 'Pilih Tipe User'" />
                                        </SelectTrigger>
                                        <SelectContent>
                                ...
                                <AlertDialogContent class="p-4">
                                    ...
                                    <AlertDialogFooter>
                                        ...
                                        <AlertDialogAction as-child>
                                            <Button type="submit" form="userForm">Ya, Perbarui</Button>
                                        </AlertDialogAction>
                                    </AlertDialogFooter>
                                </AlertDialogContent>
                            </AlertDialog>
